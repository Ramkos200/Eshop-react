<nav x-data="{ open: false }" class="bg-white/10 backdrop-blur-md border-b border-white/10 shadow-lg">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}"
                        class="font-['Cormorant_Garamond'] text-2xl font-light text-white text-shadow-lg shadow-white/10">
                        Lumière
                    </a>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            class="inline-flex items-center px-4 py-2 border border-white/20 text-sm leading-4 font-medium rounded-md text-white bg-white/10 hover:bg-white/20 focus:outline-none transition ease-in-out duration-150">
                    ...
                    <x-slot name="content">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <x-dropdown-link :href="route('logout')" class="text-gray-800 hover:bg-gray-100"
    ...
    <div :class="{ 'block': open, 'hidden': !open }"
        class="hidden sm:hidden bg-white/10 backdrop-blur-md border-b border-white/10">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-white hover:bg-white/10">
                {{ __('Dashboard') }}
            </x-responsive-nav-link>
            {{-- Commented out for now - add these when you create the routes
            <x-responsive-nav-link :href="route('products.index')" :active="request()->routeIs('products.*')" class="text-white hover:bg-white/10">
                {{ __('Products') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('categories.index')" :active="request()->routeIs('categories.*')" class="text-white hover:bg-white/10">
                {{ __('Categories') }}
            </x-responsive-nav-link>
            --}}
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-white/10">
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <x-responsive-nav-link :href="route('logout')" class="text-white hover:bg-white/10"
                        onclick="event.preventDefault();
                                        this.closest('form').submit();">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
